...
async function notifyQueueIfNeeded(
  ...
  // Only the first transition into 'scored' should notify; re-scoring an
  // already-scored class (rare) must not re-alert.
  ...
  try {
    const { data: ci } = await supabase
      ...
      .single()
    if (!ci) return
    // The BEFORE-insert/update auto-approve trigger has already run by now;
    // a NULL admin_approved_at means a coach-recorded class that still needs
    // a human in the admin queue.
...
async function processClassInput(supabase: any, payload: any): Promise<void> {
  ...
  const now = new Date()
  // Capture whether this class was already scored before this run, so the
  // queue notification only fires on the first transition into 'scored'.
  ...
  // 5. Real-time queue alert: if this freshly-scored class still needs admin
  //    review (coach-recorded, not auto-approved), ping Telegram.
