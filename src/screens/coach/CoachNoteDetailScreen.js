      ...
      <Modal visible animationType="fade" transparent onRequestClose={onClose}>
        <View style={vm.overlay}>
          <View style={vm.errorBox}>
            <Text style={vm.errorText}>Video playback not available.</Text>
...
function StudentPickerModal({ visible, onClose, onSelect, currentId }) {
  const [students, setStudents] = useState([]);
  const [search, setSearch] = useState('');

  useEffect(() => {
    if (!visible) return;
    getMyStudents().then((s) => setStudents(s || [])).catch(() => setStudents([]));
  }, [visible]);
      ...
      <View style={picker.overlay}>
        <View style={picker.sheet}>
          <View style={picker.header}>
            <Text style={picker.title}>Link to student</Text>
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => onSelect({ id: item.id, name: item.name, photoUrl: item.photoUrl })}
                  activeOpacity={0.7}
                >
                  {item.photoUrl ? (
                    ...
                    <View style={[picker.itemAvatar, picker.itemAvatarFallback]}>
                      <Text style={picker.itemAvatarText}>{item.name?.[0]?.toUpperCase() || 'S'}</Text>
                    </View>
                  )}
                  <View style={{ flex: 1 }}>
                  ...
                  {isSelected && <Ionicons name="checkmark" size={18} color={Colors.orange} />}
                </TouchableOpacity>
              );
            }}
            ListEmptyComponent={
              <Text style={picker.empty}>No students found.</Text>
            }
          />
        </View>
      </View>
...
export default function CoachNoteDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (!isNew) {
      getCoachNoteById(noteId).then((note) => {
        if (note) {
          setTitle(note.title || '');
    ...
    Alert.alert('Delete note', 'Are you sure you want to delete this note?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          if (idRef.current) await deleteCoachNote(idRef.current);
          navigation.goBack();
        },
      },
    ]);
  ...
  function handleLinkSelect(student) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      >
        ...
        >
          ...
          {linkedStudent && (
            <View style={styles.linkedRow}>
              {linkedStudent.photoUrl ? (
                <Image source={{ uri: linkedStudent.photoUrl }} style={styles.linkedAvatar} />
              ) : (
                <View style={[styles.linkedAvatar, styles.linkedAvatarFallback]}>
                  <Text style={styles.linkedAvatarText}>
                    {linkedStudent.name?.[0]?.toUpperCase() || 'S'}
                  </Text>
                </View>
              )}
              <TouchableOpacity
              ...
              >
                <Text style={styles.linkedText} numberOfLines={1}>
                  Linked to <Text style={styles.linkedName}>{linkedStudent.name}</Text>
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
        ...
        <View style={styles.bottomBar}>
          ...
          <TouchableOpacity style={styles.bottomAction} onPress={() => setPickerVisible(true)} activeOpacity={0.7}>
            <Ionicons
              name={linkedStudent ? 'person' : 'person-outline'}
              size={18}
              color={linkedStudent ? Colors.orange : Colors.secondary}
            />
            <Text style={[styles.bottomActionText, linkedStudent && styles.bottomActionLinked]}>
              {linkedStudent ? 'Student Linked' : 'Link Student'}
            </Text>
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
      ...
      <StudentPickerModal
        ...
        currentId={linkedStudent?.id || null}
      />
